// Cloudflare Function for /api/photos/featured
import { d1Database } from '../../../lib/d1-db';

export async function onRequestPUT(context: any): Promise<Response> {
  console.log('Featured photo PUT request received');
  ...
  try {
    ...
    console.log('Photo ID from query:', photoId);
    
    if (!photoId) {
      return new Response(
        JSON.stringify({ error: 'photoId query parameter is required' }),
        { 
          status: 400,
